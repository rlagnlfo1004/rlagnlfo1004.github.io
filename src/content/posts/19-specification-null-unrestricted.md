...
갈라진 지점을 그림으로 두면 이렇습니다.

<div class="diagram-scroll" style="--diagram-min-w: 680px">
<svg class="diagram" viewBox="0 0 720 300" role="img" aria-label="3.5 에서는 두 종류의 null 이 같은 결과로 수렴했고 4.0 에서는 참조 null 만 막혔다">
  <defs>
    <marker id="p19-arrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
      <path d="M0 1 L7 4 L0 7 z" fill="var(--ink-3, #9A958B)"/>
    </marker>
    <marker id="p19-arrow-clay" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
...
<p class="diagram-note">4.0 이 없앤 것은 오른쪽 위 경로 하나입니다. 아래 경로는 이름만 <code>unrestricted()</code> 로 바뀌었고 동작은 그대로예요.</p>
