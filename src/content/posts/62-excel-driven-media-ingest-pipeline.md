...
<p class="diagram-note">274MB 는 잰 값이 아니라 실측 원본 137MB 를 워커 수로 곱한 산술입니다. 아래 막대의 복사 버퍼 폭은 눈에 보이게 그린 것이고 실제 비율이 아닙니다. 8192바이트를 이 눈금에 맞추면 0.004픽셀이라 그릴 수가 없어요.</p>
...
`Files.copy(body, tempFile)` 한 줄이 실제로 잡는 힙이 얼마인지 JDK 17 소스로 확인해 봤습니다. 이 메서드는 마지막에 `in.transferTo(out)` 을 부르고, 그 기본 구현은 배열 하나를 잡아 끝까지 돌려 써요.

```java
// java.io.InputStream (JDK 17)
...
**8192바이트예요.** 원본이 137MB 든 1GB 든 이 숫자는 안 변합니다. 소켓과 파일 스트림이 각자 갖는 버퍼가 더 있지만 전부 KB 단위이고, 무엇보다 **원본 용량에 비례해 늘지 않아요.** 앞의 배열 방식과 갈리는 지점이 정확히 여기입니다. 한쪽은 힙 사용량이 입력에 비례하고, 다른 쪽은 상수예요.
...
값을 한 자리가 하나 더 있습니다. 출력 경로를 실측해 보니 **PBF 가 만든 잡과 CLI 로 직접 만든 잡이 서로 달랐어요.**

```
CLI 로 만든 잡    output/{입력 객체 키}/master.m3u8
PBF 가 만든 잡    pbf/output/{입력 객체 키}/{확장자 뺀 입력 객체 키}/master.m3u8
```

PBF 가 `basePrefix` 를 `pbf/output/` 아래로 잡고 입력 키를 한 번 더 반복합니다. 경로를 규칙으로 조립했다면 그대로 틀렸을 텐데, 잡 응답의 `outputs[]` 에 적힌 실제 경로를 쓰기 때문에 코드를 한 줄도 고치지 않았습니다. 조립 규칙은 `outputs` 를 못 뽑았을 때의 폴백으로만 남아 있어요.

그 `outputs[]` 에도 함정이 있었습니다.

```java
for (JobOutput output : outputs) {
    if (!java.util.Objects.equals(outputBucket, output.getBucketName())) {
        continue;
    }
    // ...
}
...
**첫 항목이 출력이 아니라 원본입니다**(`assetType=INPUT_FILE`). `outputs[0]` 을 집으면 재생 URL 자리에 원본 mp4 경로가 저장돼요. 그래서 인덱스로 고르지 않고 출력 버킷 이름으로 걸렀고, 테스트로 잠가 뒀습니다.

입력 객체 키를 찾는 쪽은 한 번 더 방어적으로 갔습니다. 어떤 파라미터 이름에 담기는지가 실측 전이라 **이름이 아니라 값의 모양으로** 찾아요. 우리 원본 키는 전부 `source/` 로 시작하니, 잡 파라미터를 재귀로 훑어 그 프리픽스로 시작하는 문자열을 집습니다. 못 찾으면 로그를 남기고 상태를 바꾸지 않습니다.
...
되돌린 행을 다른 워커가 집었는데 원래 워커가 아직 살아 있는 경우예요. 그래서 상태 전이 메서드를 전부 조건부로 만들었습니다.

```java
public void markUploaded() {
    if (this.encodingStatus == MediaEncodingStatus.INGESTING) {   // 무조건 전이가 아니다
        this.encodingStatus = MediaEncodingStatus.UPLOADING;
    }
}
```

`markIngesting` 과 `releaseToPending` 도 같은 모양이에요. **선점만 조건부인 게 아니라 전이 전체가 조건부입니다.** 상태를 바꾸는 코드가 소스 상태를 확인하지 않으면 늦게 도착한 워커가 이미 지나간 상태로 되돌려 놓습니다.

다만 이 가드가 지키는 것은 **상태값**이고, 부작용은 못 막아요. 코드를 따라가면 이렇게 됩니다.

1. A 가 선점해 다운로드를 시작하고 10분을 넘긴다
2. 회수가 그 행을 `PENDING` 으로 되돌린다
3. B 가 선점해 같은 링크를 다시 받는다
4. A 가 늦게 끝나 `PutObject` 를 부른다. 객체 키가 그대로라 덮어쓰기가 된다
5. B 도 끝나 `PutObject` 를 부른다

`markUploaded` 는 둘 중 먼저 온 쪽만 통과시키니 상태는 어긋나지 않습니다. 그런데 `createobject` 이벤트가 두 번 발화하니 **인코딩 잡이 두 개 생겨요.** 같은 원본을 두 번 인코딩하는 것이라 결과 영상은 같지만 요금은 두 배입니다. 완료 콜백도 두 번 오는데 `markDone` 이 두 번째에 `false` 를 돌려주니 알림은 한 번만 울리고, `ma_oci_job_id` 에는 먼저 도착한 잡 하나만 남아요.

관측한 적은 없고 코드를 따라간 시나리오입니다. 그래도 **고아 임계값을 정할 때 저울에 올릴 비용이 "잡 하나 값"이라는 건 분명해요.** 10분보다 짧게 잡으면 이 경우가 늘고, 길게 잡으면 진짜로 죽은 워커가 쥔 행이 오래 묶입니다.
...
그리고 이건 예외 상황이 아니라 **상시 동작입니다.** 숫자를 맞춰 보면 드러나요. 엑셀 상한은 500행인데 큐는 200이니까요.

| 값 | 어디에 박혀 있나 |
|---|---|
| 엑셀 한 장 500행 | `LearningVideoExcelPolicy.MAX_ROWS` |
| 실행 2 + 큐 200 = 202건 수용 | `AsyncConfig` |
| 나머지 298건 즉시 폐기 | `DiscardPolicy` |
| 한 주기 회수 100건 | `media.ingest.pending-batch-size` |

500행을 한 번에 접수하면 202건만 받아들이고 **298건은 그 자리에서 버립니다.** 접수 루프가 도는 동안 반입 한 건이 끝날 일은 없으니(실측이 8초와 17초였어요) 이 산술이 거의 그대로 나올 거예요. 그래서 처음에는 큐를 500으로 올릴 생각을 했습니다. 상한과 맞춰야 깔끔해 보였으니까요.

올리지 않았어요. **큐를 늘려도 처리 속도는 워커 2개가 정하기 때문입니다.** 반입 한 건이 8초에서 17초였으니 분당 7건에서 15건이고, 500편이면 33분에서 70분입니다(실측 소요를 워커 수로 나눈 산술이에요). 큐를 500으로 늘리면 대기열이 힙 위 자바 객체로 앉아 있는 시간만 길어지고, 그동안 어느 편이 어디까지 갔는지를 DB 가 아니라 큐가 알고 있게 됩니다. 인스턴스가 재시작하면 그 큐는 사라져요.

그래서 큐는 좁게 두고 **DB 를 진짜 큐로 씁니다.** 200은 "이만큼은 즉시 시작해 본다"는 완충이고, 순서와 진행은 `hama_media_asset` 의 상태 컬럼이 갖습니다. 버려진 298건은 60초마다 100건씩 다시 신청하고, 큐가 비는 만큼만 들어가요.
...
**다섯째, 시프트 폭을 10 으로 막았습니다.** `1L << Math.min(attempt, 10)` 이에요. 지금은 시도가 3회라 닿지 않는 가드지만 `max-attempts` 가 설정값이라, 누가 40 으로 올리면 `1L << 40` 이 계산됩니다. 30초 상한이 뒤에서 잘라 주긴 해도 상한을 구하는 계산 자체가 넘치지 않게 막아 두는 쪽이 맞다고 봤어요.

이 값들로 실제 대기가 얼마인지 세어 보면 이렇습니다. `max-attempts: 3` 이면 **잠드는 것은 두 번뿐이에요.** 마지막 시도 뒤에는 `attempt == maxAttempts - 1` 에서 빠져나가니까요. 그러니 최악이 `0~1초` 와 `0~2초` 를 더한 **3초**이고 기대값은 그 절반인 1.5초입니다. 링크 하나가 실패로 굳기까지 3초를 넘게 기다리지 않는다는 뜻이라, 500행을 훑는 워커가 실패 한 건에 오래 묶이지 않아요.
...
**부하 제어는 때리는 속도와 물러나는 방식을 나눠 짰습니다.** 동시 2 와 큐 200 에 `DiscardPolicy` 를 걸어 상한을 잡고, 재시도는 Full Jitter 로 흩었어요. 500행을 올리면 298건이 정상적으로 버려지고 스케줄러가 되돌립니다. 버려도 되는 이유는 큐가 아니라 DB 가 큐이기 때문이에요.

남은 것도 적어 둡니다. 고아 임계값 10분은 "정상 반입이 10분 안에 끝난다"는 가정 위에 있고, 그 가정은 137MB 를 8초에 받은 한 번의 관측으로 세웠습니다. 500MB 짜리가 느린 회선으로 들어오면 반입 중인 행을 고아로 오인해 되돌릴 수 있어요. 그때 상태값은 조건부 전이가 지켜 주지만 인코딩 잡이 두 개 생기는 것은 못 막습니다. `UPLOADING` 에서 잡이 영원히 안 끝나는 경우를 감지하는 장치도 아직 없습니다. 217편 1회성 적재라 사람 눈이 있다고 보고 미뤘는데, 상시 운영으로 넘어가면 그때 다시 봐야 할 자리입니다.
